...
def show_comparison_summary(comparison_json):
    ...
    try:
        ...
        if 'page_load_time' in kpi:
            ...
            direction = "[UP]" if plt.get('direction') == 'increased' else "[DOWN]" if plt.get('direction') == 'decreased' else "[SAME]"
            print(f"Page Load Time: {plt.get('base', 0):.1f}s → {plt.get('target', 0):.1f}s {direction} ({plt.get('percentage', 0):+.1f}%)")
        
        if 'total_requests' in kpi:
            tr = kpi['total_requests']
            direction = "[UP]" if tr.get('direction') == 'increased' else "[DOWN]" if tr.get('direction') == 'decreased' else "[SAME]"
            print(f"Total Requests: {tr.get('base', 0)} → {tr.get('target', 0)} {direction} ({tr.get('percentage', 0):+.1f}%)")
        
        if 'total_size_mb' in kpi:
            ts = kpi['total_size_mb']
            direction = "[UP]" if ts.get('direction') == 'increased' else "[DOWN]" if ts.get('direction') == 'decreased' else "[SAME]"
            print(f"Total Size: {ts.get('base', 0):.1f}MB → {ts.get('target', 0):.1f}MB {direction} ({ts.get('percentage', 0):+.1f}%)")
...
def main():
    ...
    baseline_har = select_har_file("SELECT BASELINE HAR FILE (reference point):", har_files)
    if not baseline_har:
        print("No baseline file selected. Exiting.")
        return
    
    # Select target HAR file
    target_har = select_har_file("SELECT TARGET HAR FILE (to compare against baseline):", har_files, baseline_har)
    if not target_har:
    ...
    report_name = f"comparison_report_{baseline_har.stem}_vs_{target_har.stem}_{timestamp}.html"
