    ...
    while True:
        ...
        try:
            ...
            if any(i < 1 or i > len(har_files) for i in indices):
                print(
                    f"❌ Invalid selection! Please choose numbers between 1 and {len(har_files)}"
                )
                print()
                continue
...
def run_analysis(selected_files, report_type, output_file):
    ...
    # Prepare command
    cmd = [sys.executable, "scripts/generate_multi_har_report.py"]
...
def main():
    ...
    while True:
        ...
        while True:
            continue_choice = (
                input("🔄 Analyze another set of files? (y/n): ").strip().lower()
            )
