...
class MultiRunReportGenerator:
    ...
    def generate_report(self, har_files: List[Path], output_path: Path, 
        ...
        try:
            ...
            logger.info(f"Generating {report_type} multi-run report for {len(har_files)} HAR files")
        ...
        for i, har_file in enumerate(har_files):
            try:
                ...
                if not chunk_dir.exists():
                    logger.info(f"Chunked data not found for {har_file.stem}, creating chunks...")
                    self._create_chunks_for_har(har_file)
                
                # Load chunked data using existing break_har_for_single_analysis.py structure
                processed_data = self._load_chunked_data(chunk_dir, har_file.stem, i + 1)
                if processed_data:
    ...
    def _create_chunks_for_har(self, har_file: Path) -> bool:
        ...
        try:
            ...
            chunk_dir = Path("har_chunks") / har_file.stem
            break_har_for_single_analysis.main(har_file=str(har_file), output_dir=str(chunk_dir))
    ...
    def _load_chunked_data(self, chunk_dir: Path, run_name: str, run_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if processed["summary_data"]:
                ...
                total_size = sum(max(0, req.get("size", 0)) for req in requests)  # Fix negative sizes
                total_requests = len(requests)
                
                processed["analysis_summary"] = {
                    "total_requests": total_requests,
                    "total_size": total_size,
                    "failed_requests": len([r for r in requests if r.get("status", 0) >= 400])
                }
    ...
    def _process_har_data(self, har_data: Dict[str, Any], run_name: str, run_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for entry in entries:
                ...
                request_data = {
                    ...
                    "size": response.get("bodySize", 0) + response.get("headersSize", 0),
                ...
                resource_type = self._categorize_resource_type(mime_type, request_data["url"])
                resource_types[resource_type] = resource_types.get(resource_type, {"count": 0, "size": 0})
    ...
    def _perform_analysis(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        timing_comparison = self.performance_comparator.compare_timing_metrics(runs_data)
        resource_comparison = self.performance_comparator.compare_resource_metrics(runs_data)
        network_comparison = self.performance_comparator.compare_network_metrics(runs_data)
        ...
        analysis_results["cross_run_patterns"] = self.multi_run_analyzer.analyze_cross_run_patterns(runs_data)
        
        # Resource consistency analysis
        analysis_results["resource_analysis"] = self.multi_run_analyzer.analyze_resource_consistency(runs_data)
        
        # Third-party impact analysis
        analysis_results["network_analysis"] = self.multi_run_analyzer.analyze_third_party_impact(runs_data)
        
        # Performance bottleneck identification
        analysis_results["bottlenecks"] = self.multi_run_analyzer.identify_performance_bottlenecks(runs_data)
        
        # Generate recommendations
        analysis_results["recommendations"] = self.performance_comparator.generate_recommendations(
    ...
    def _generate_html_report(self, analysis_results: Dict[str, Any], report_type: str) -> str:
    ...
    def _populate_template(self, template: str, analysis_results: Dict[str, Any]) -> str:
    ...
    def _generate_executive_summary(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        timing_data = analysis_results.get("performance_comparison", {}).get("timing", {})
        if timing_data.get("performance_scores"):
            avg_score = statistics.mean([score["score"] for score in timing_data["performance_scores"]])
        ...
        if consistency_ratings:
            ...
            avg_consistency = statistics.mean([rating_scores.get(r, 0) for r in consistency_ratings])
        ...
        summary["top_recommendations"] = [rec["recommendation"] for rec in all_recommendations[:3]]
    ...
    def _calculate_third_party_size(self, entries: List[Dict]) -> int:
        ...
        return sum(entry.get("response", {}).get("bodySize", 0) for entry in entries) // 3
    ...
    def _analyze_third_party_providers(self, entries: List[Dict]) -> Dict[str, Any]:
        ...
        for entry in entries:
            ...
            if self._is_likely_third_party(domain):
                ...
                providers[domain]["size"] += entry.get("response", {}).get("bodySize", 0)
    ...
    def _is_likely_third_party(self, domain: str) -> bool:
        ...
        third_party_indicators = [
            ...
            "analytics", "tracking", "ads", "cdn", "cloudflare"
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate multi-run HAR analysis report")
    parser.add_argument("har_files", nargs="+", help="HAR files to analyze (2-10 files)")
    parser.add_argument("-o", "--output", required=True, help="Output HTML file path")
    parser.add_argument("-t", "--type", choices=["comprehensive", "dashboard", "executive"], 
