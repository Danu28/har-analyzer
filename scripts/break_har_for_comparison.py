...
def extract_har_data(har_file_path: str) -> Dict[str, Any]:
    ...
    for i, entry in enumerate(entries):
        ...
        resource_type = _determine_resource_type(
            url, mime_type, entry.get("_resourceType")
        )
    ...
    print(
        f"[OK] Processed {len(requests)} requests across {len(resource_types)} resource types"
    )
    return structured_data


def _determine_resource_type(
    url: str, mime_type: str, har_resource_type: Optional[str] = None
) -> str:
    ...
    if any(ext in url_lower for ext in [".js", ".jsx", ".ts", ".tsx"]):
        ...
        ext in url_lower for ext in [".mp4", ".avi", ".mov", ".wmv", ".mp3", ".wav"]
    ):
        return "media"
...
def _calculate_performance_metrics(
    requests: List[Dict], page_timings: Dict, total_size: int
) -> Dict[str, Any]:
    ...
    connect_times = [
        req["timings"]["connect"] for req in requests if req["timings"]["connect"] > 0
    ]
...
def save_broken_har_data(broken_data: Dict[str, Any], output_dir: str) -> str:
    ...
    components = {
        ...
        "resource_types.json": {
            k: v[:10] for k, v in broken_data["resource_types"].items()
        },  # Limit per type
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Break down HAR files for comparison analysis"
    )
    parser.add_argument("--har", required=True, help="Path to HAR file")
    parser.add_argument(
        "--output", help="Output directory (default: har_breakdown_<filename>)"
    )
