    ...
    if not har_files:
        ...
        if Path(root_dir, "HAR-Files").exists():
            ...
            for f in Path(root_dir, "HAR-Files").iterdir():
                print(f"  - {f.name}")
        print_info(
            "Please ensure a .har file exists in the current directory or HAR-Files subdirectory"
        )
        print_info(
            "Or specify the path manually: python break_har_file.py --har path/to/file.har"
        )
        sys.exit(1)
...
def main(har_file=None, output_dir=None):
    ...
    with open(
        os.path.join(output_dir, "01_header_and_metadata.json"), "w", encoding="utf-8"
    ) as f:
        ...
        summary.append(
            {
                ...
                "timings": entry.get(
                    "timings", {}
                ),  # Include timings for DNS/SSL analysis
            }
        )
    summary_data = {"totalEntries": len(summary), "requests": summary}
    with open(
        os.path.join(output_dir, "02_requests_summary.json"), "w", encoding="utf-8"
    ) as f:
    ...
    for rtype, reqs in resource_types.items():
        resource_data = {"resourceType": rtype, "count": len(reqs), "requests": reqs}
    ...
    with open(
        os.path.join(output_dir, "00_index_and_guide.json"), "w", encoding="utf-8"
    ) as f:
    ...
    readme_lines = [
        ...
        + "\n".join(
            [
                f"- **04_resource_type_{rtype.lower()}.json** - {len(reqs)} {rtype} requests"
                for rtype, reqs in resource_types.items()
            ]
        )
        + "\n",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Break HAR file into chunks for single analysis"
    )
    parser.add_argument("--har", dest="har_file", default=None, help="Path to HAR file")
    parser.add_argument(
        "--output", dest="output_dir", default=None, help="Output directory for chunks"
    )
