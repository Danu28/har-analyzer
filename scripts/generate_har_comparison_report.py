...
def generate_comparison_report(
    ...
    if template_file and os.path.exists(template_file):
        ...
        template_filename = (
            f"har_comparison_{template_style.replace('-', '_')}.html"
            if template_style != "detailed"
            else "har_comparison_expected.html"
        )
        default_template = (
...
def _process_comparison_data(comparison_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    processed = {
        ...
        "failed_requests_comparison": comparison_data.get(
            "failed_requests_comparison", {}
        ),
    }

    # Add current timestamp to metadata
    if "comparison_timestamp" not in processed["metadata"]:
        processed["metadata"]["comparison_timestamp"] = datetime.now().strftime(
            "%Y-%m-%d %H:%M:%S"
        )
...
def _render_simple_template(template_content: str, data: Dict[str, Any]) -> str:
    ...
    html = html.replace(
        "{{ metadata.base_file }}", metadata.get("base_file", "base.har")
    )
    html = html.replace(
        "{{ metadata.target_file }}", metadata.get("target_file", "target.har")
    )
    html = html.replace(
        "{{ metadata.comparison_timestamp }}", metadata.get("comparison_timestamp", "")
    )

    # Replace summary data
    summary = data.get("summary", {})
    html = html.replace(
        "{{ summary.overall_status }}", summary.get("overall_status", "unknown")
    )
    html = html.replace(
        "{{ summary.risk_level }}", summary.get("risk_level", "unknown")
    )
    ...
    if "page_load_time" in kpi:
        ...
        html = html.replace(
            "{{ kpi_changes.page_load_time.target }}", str(plt.get("target", 0))
        )
        html = html.replace(
            "{{ kpi_changes.page_load_time.absolute }}", str(plt.get("absolute", 0))
        )
        html = html.replace(
            "{{ kpi_changes.page_load_time.percentage }}", str(plt.get("percentage", 0))
        )
...
def create_comparison_workflow(
    base_har: str, target_har: str, output_dir: str = None
) -> str:
    """
    ...
    Returns:
        Path to generated HTML report
    """
    from . import analyze_two_chunks, break_har
...
if __name__ == "__main__":
    ...
    report_parser = subparsers.add_parser(
        "report", help="Generate report from comparison JSON"
    )
    report_parser.add_argument(
        "--comparison", required=True, help="Comparison analysis JSON file"
    )
    report_parser.add_argument("--output", help="Output HTML file")
    report_parser.add_argument("--template", help="Custom template file")
    report_parser.add_argument(
        "--template-style",
        choices=["side-by-side", "detailed"],
    ...
    workflow_parser = subparsers.add_parser(
        "workflow", help="Complete comparison workflow"
    )
    ...
    try:
        if args.mode == "report":
            ...
            report_file = create_comparison_workflow(
                args.base, args.target, args.output_dir
            )
