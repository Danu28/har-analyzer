...
class MultiRunAnalyzer:
    ...
    def analyze_cross_run_patterns(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def analyze_resource_consistency(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if all_urls:
            ...
            consistency_analysis["core_resources"] = {
                "count": len(core_urls),
                "percentage": (len(core_urls) / len(all_unique_urls) * 100) if all_unique_urls else 0,
                "urls": list(core_urls)
            }
            ...
            consistency_analysis["variable_resources"] = {
                ...
                "percentage": (len(variable_urls) / len(all_unique_urls) * 100) if all_unique_urls else 0,
                "urls": list(variable_urls)
            }
            
            # Calculate consistency score
            consistency_analysis["consistency_score"] = (len(core_urls) / len(all_unique_urls) * 100) if all_unique_urls else 100
        
        # Analyze resource stability by domain
        consistency_analysis["resource_stability"] = self._analyze_resource_stability_by_domain(all_urls_by_run)
        
        return consistency_analysis
    
    def analyze_third_party_impact(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for provider in all_providers:
            appearances = sum(1 for run in third_party_by_run if provider in run["providers"])
        ...
        third_party_analysis["consistent_providers"] = {
            ...
            "impact": self._calculate_provider_impact(consistent_providers, third_party_by_run)
        }
        
        third_party_analysis["variable_providers"] = {
            "count": len(variable_providers),
            "providers": list(variable_providers),
            "impact": self._calculate_provider_impact(variable_providers, third_party_by_run)
        }
        
        # Generate third-party optimization recommendations
        third_party_analysis["recommendations"] = self._generate_third_party_recommendations(
            consistent_providers, variable_providers, third_party_by_run
        )
        
        return third_party_analysis
    
    def identify_performance_bottlenecks(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        bottlenecks["slow_resources"] = [
            ...
            for url, time in sorted(consistent_slow.items(), key=lambda x: x[1], reverse=True)[:10]
        ]
        
        # Analyze large resources
        bottlenecks["large_resources"] = self._identify_large_resources_across_runs(runs_data)
        
        # Analyze inefficient domains
        bottlenecks["inefficient_domains"] = self._identify_inefficient_domains(runs_data)
    ...
    def _analyze_url_consistency(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if all_run_urls:
            ...
            url_analysis.update({
                ...
                "consistency_percentage": (len(consistent) / len(all_unique) * 100) if all_unique else 100
            })
    ...
    def _analyze_resource_patterns(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _analyze_third_party_consistency(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _analyze_timing_patterns(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _analyze_error_patterns(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for url in all_error_urls:
            appearances = sum(1 for run in errors_by_run 
            ...
            if appearances == len(runs_data):
                ...
                error_analysis["intermittent_errors"].append({
                    "url": url,
                    "frequency": f"{appearances}/{len(runs_data)}"
                })
    ...
    def _analyze_cache_behavior(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _extract_third_party_providers(self, run_data: Dict[str, Any]) -> Dict[str, int]:
    ...
    def _extract_errors_from_run(self, run_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _identify_large_resources_across_runs(self, runs_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _identify_inefficient_domains(self, runs_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        return sorted(inefficient_domains, key=lambda x: x["avg_response_time"], reverse=True)[:10]
    ...
    def _analyze_resource_stability_by_domain(self, all_urls_by_run: List[Dict]) -> Dict[str, Any]:
        ...
        for domain in all_domains:
            ...
            if domain_urls_all_runs:
                ...
                consistent_urls = set.intersection(*domain_urls_all_runs) if len(domain_urls_all_runs) > 1 else all_domain_urls
                
                stability_score = (len(consistent_urls) / len(all_domain_urls) * 100) if all_domain_urls else 100
    ...
    def _calculate_provider_impact(self, providers: Set[str], third_party_by_run: List[Dict]) -> Dict[str, Any]:
        """Calculate the impact of third-party providers."""
        impact = {
            "total_requests": 0,
            "total_size": 0,
            "avg_requests_per_run": 0
        }
        ...
        impact.update({
            ...
            "avg_requests_per_run": total_requests / len(third_party_by_run) if third_party_by_run else 0
        })
        
        return impact
    
    def _generate_third_party_recommendations(self, consistent_providers: Set[str], 
                                            variable_providers: Set[str], 
                                            third_party_by_run: List[Dict]) -> List[str]:
        """Generate recommendations for third-party optimization."""
        recommendations = []
        
        if len(consistent_providers) > 10:
            recommendations.append("Consider consolidating third-party providers to reduce DNS lookups and connection overhead")
        
        if len(variable_providers) > len(consistent_providers):
            recommendations.append("High variability in third-party services may indicate A/B testing or conditional loading")
        
        # Calculate average impact
        total_third_party_requests = sum(run["total_requests"] for run in third_party_by_run)
        avg_requests = total_third_party_requests / len(third_party_by_run) if third_party_by_run else 0
        
        if avg_requests > 50:
            recommendations.append("High third-party request volume - consider lazy loading or request prioritization")
    ...
    def _generate_bottleneck_recommendations(self, bottlenecks: Dict[str, Any]) -> List[str]:
        ...
        if bottlenecks["slow_resources"]:
            recommendations.append("Optimize consistently slow resources through compression, CDN, or caching")
        ...
        if bottlenecks["inefficient_domains"]:
            recommendations.append("Consider self-hosting or finding alternatives for slow third-party domains")
