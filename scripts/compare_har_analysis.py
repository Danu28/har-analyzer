...
def compare_har_chunks(
    base_data: Dict[str, Any], target_data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    print(
        f"[BASE] Base: {base_name} ({base_data['totals']['total_requests']} requests)"
    )
    print(
        f"[TARGET] Target: {target_name} ({target_data['totals']['total_requests']} requests)"
    )
    ...
    comparison = {
        ...
        "performance_regression": _detect_performance_regression(
...
def _compare_kpis(base_data: Dict, target_data: Dict) -> Dict[str, Any]:
    ...
    def calculate_change(base_val, target_val, is_percentage=False):
        ...
        direction = (
            ...
            else "decreased" if absolute_change < 0 else "unchanged"
        )
    ...
    return {
        ...
        "total_requests": calculate_change(
            base_totals["total_requests"], target_totals["total_requests"]
        ),
        "total_size_mb": calculate_change(
            base_totals["total_size_mb"], target_totals["total_size_mb"]
        ),
        "failed_requests": calculate_change(
            base_totals["failed_count"], target_totals["failed_count"]
        ),
        "slow_requests": calculate_change(
            base_totals["slow_count"], target_totals["slow_count"]
        ),
...
def _compare_endpoint_timings(base_data: Dict, target_data: Dict) -> Dict[str, Any]:
    ...
    for url in common_urls:
        ...
        if abs(time_delta) > 10 or (
            base_req["time"] > 0 and abs(time_delta / base_req["time"]) > 0.1
        ):
            timing_comparisons.append(
                {
                    ...
                    "percentage_change": (
                        round((time_delta / base_req["time"]) * 100, 2)
                        if base_req["time"] > 0
                        else "N/A"
                    ),
    ...
    return {
        ...
        "summary": {
            ...
            "improved_count": len(
                [c for c in timing_comparisons if c["time_delta"] < 0]
            ),
            "regressed_count": len(
                [c for c in timing_comparisons if c["time_delta"] > 0]
            ),
...
def _compare_resource_types(base_data: Dict, target_data: Dict) -> Dict[str, Any]:
    ...
    for resource_type in all_types:
        base_stats = base_breakdown.get(
            resource_type, {"count": 0, "size": 0, "time": 0}
        )
        target_stats = target_breakdown.get(
            resource_type, {"count": 0, "size": 0, "time": 0}
        )
        ...
        comparisons[resource_type] = {
            ...
            "size": {
                ...
                "delta_mb": round(
                    (target_stats["size"] - base_stats["size"]) / (1024 * 1024), 2
                ),
            },
            "time": {
...
def _detect_performance_regression(
    base_data: Dict, target_data: Dict
) -> Dict[str, Any]:
    ...
    return {
        ...
        "overall_status": (
            "regression" if regressions else "improvement" if improvements else "stable"
        ),
    }
    ...
    return {
        ...
        "domain_changes": {
            domain: {
                ...
                "target": target_domains.get(
                    domain, {"count": 0, "size": 0, "time": 0}
                ),
            }
...
def _analyze_size_changes(base_data: Dict, target_data: Dict) -> Dict[str, Any]:
    ...
    return {
        ...
        "largest_increase": (
            max(size_changes, key=lambda x: x["size_delta"]) if size_changes else None
        ),
        "largest_decrease": (
            min(size_changes, key=lambda x: x["size_delta"]) if size_changes else None
        ),
    }
...
def _generate_comparison_summary(comparison: Dict) -> Dict[str, Any]:
    ...
    overall_status = (
        "improved" if status_score > 0 else "regressed" if status_score < 0 else "mixed"
    )
    ...
    return {
        ...
        "risk_level": (
            "high"
            if len(regression["regressions"]) > 3
            else "medium" if len(regression["regressions"]) > 0 else "low"
        ),
    }
...
def save_comparison_analysis(
    comparison: Dict[str, Any], output_file: str = None
) -> str:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--target", required=True, help="Target HAR breakdown JSON file"
    )
