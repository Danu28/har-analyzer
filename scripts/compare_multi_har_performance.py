...
class PerformanceComparator:
    ...
    def compare_timing_metrics(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        metric_names = ["page_load_time", "dom_ready_time", "first_contentful_paint", 
                       "largest_contentful_paint", "total_blocking_time"]
        
        for metric in metric_names:
            values = [run.get(metric, 0) for run in metrics_by_run if run.get(metric) is not None]
            if values:
                timing_comparison["metrics"][metric] = {
                    ...
                    "variation_coefficient": (statistics.stdev(values) / statistics.mean(values)) * 100 if len(values) > 1 and statistics.mean(values) > 0 else 0,
        ...
        timing_comparison["consistency"]["overall_rating"] = self._calculate_overall_consistency(timing_comparison["metrics"])
        timing_comparison["consistency"]["most_consistent"] = self._find_most_consistent_metric(timing_comparison["metrics"])
        timing_comparison["consistency"]["least_consistent"] = self._find_least_consistent_metric(timing_comparison["metrics"])
        
        return timing_comparison
    
    def compare_resource_metrics(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for res_type in resource_types:
            type_counts = [run["by_type"].get(res_type, {}).get("count", 0) for run in runs_resources]
            type_sizes = [run["by_type"].get(res_type, {}).get("size", 0) for run in runs_resources]
        ...
        resource_comparison["efficiency"] = self._analyze_resource_efficiency(runs_resources)
        
        return resource_comparison
    
    def compare_network_metrics(self, runs_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if any(time > 0 for time in ssl_times):
            network_comparison["ssl_analysis"] = {
                ...
                "consistency": self._calculate_consistency_rating([t for t in ssl_times if t > 0])
            }
        ...
        if any(time > 0 for time in dns_times):
            network_comparison["dns_analysis"] = {
                ...
                "consistency": self._calculate_consistency_rating([t for t in dns_times if t > 0])
            }
        
        return network_comparison
    
    def generate_recommendations(self, timing_comparison: Dict, resource_comparison: Dict, 
                               network_comparison: Dict) -> List[Dict[str, Any]]:
        ...
        third_party_mean = network_comparison["third_party_impact"]["request_count"]["mean"]
    ...
    def _extract_timing_metrics(self, run_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        if "analysis_summary" in run_data:
            ...
            metrics.update({
                ...
                "largest_contentful_paint": summary.get("largest_contentful_paint", 0),
    ...
    def _extract_network_metrics(self, run_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "network_analysis" in run_data:
            ...
            network.update({
                "connection_reuse_rate": net_analysis.get("connection_reuse_rate", 0),
    ...
    def _calculate_trend(self, values: List[float]) -> str:
        ...
        if slope > threshold:
            return "Improving" if any("time" in str(values) for values in [values]) else "Worsening"
        elif slope < -threshold:
            return "Worsening" if any("time" in str(values) for values in [values]) else "Improving"
        else:
            return "Stable"
    ...
    def _calculate_overall_consistency(self, metrics: Dict[str, Dict]) -> str:
        ...
        ratings = []
        rating_scores = {"Excellent": 4, "Good": 3, "Fair": 2, "Poor": 1, "Insufficient Data": 0}
    ...
    def _analyze_resource_efficiency(self, runs_resources: List[Dict]) -> Dict[str, Any]:
